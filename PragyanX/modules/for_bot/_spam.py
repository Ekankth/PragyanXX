...
from PragyanX.data import spam_usage
from PragyanX import res_grps, res_devs
from PragyanX.functions import start_spam, start_dspam, start_pspam
...
async def delayspam(PragyanX: Client, e: Message): 
    ...
    if len(Rizoelop) == 2:
       ...
       await start_dspam(PragyanX, e, counts, sleeptime, spam_text)
    else:
    ...
    if LOGS_CHANNEL:
         try:
             await PragyanX.send_message(LOGS_CHANNEL, f"started Delay Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg} \n Delay Time: {sleeptime}")
...
async def pornspam(PragyanX: Client, e: Message):
    args = e.text.split(" ", 1)[1].split(" ", 1)
    if args:
       counts = int(args[0])
       await start_pspam(PragyanX, e, counts)
       
       if LOGS_CHANNEL:
           try:
               await PragyanX.send_message(LOGS_CHANNEL, f"started Porn Spam By User: {e.from_user.id} \n Chat: {e.chat.id} \n Counts: {count}")
           except Exception as a:
               print(a)
               pass
    else:
...
async def justspam(PragyanX: Client, e: Message):
    usage = spam_usage.spam
    Rizoel = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 1)
    if len(Rizoel) == 2:
       counts = int(Rizoel[0])
       spam_text = str(Rizoel [1])
       await start_spam(PragyanX, e, counts, spam_text)
    else:
        await e.reply_text(usage.format(handler))
        return 
    if LOGS_CHANNEL:
         try:
            await PragyanX.send_message(LOGS_CHANNEL, f"started Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {counts} \n Spam Message: {msg}")
         except Exception as a:
             print(a)
             pass
...
async def hang(PragyanX: Client, e: Message): 
    counts = e.text[5:]
    if not counts:
        await e.reply_text("Gime Counts")
        return
    ...
    for _ in range(count):
         await PragyanX.send_message(e.chat.id, rizoel)
         await asyncio.sleep(0.3)
    if LOGS_CHANNEL:
         try:
            await PragyanX.send_message(LOGS_CHANNEL, f"started Hang Spam By User: {e.from_user.id} \n\n Chat: {e.chat.id} \n Counts: {count}")
         except Exception as a:
             print(a)
             pass
